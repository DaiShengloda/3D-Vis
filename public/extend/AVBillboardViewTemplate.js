...
var AVBillboardViewTemplate = function(sceneManager, id){
	this.sceneManager = sceneManager;
	this.setId(id);
	this._color = 'rgba(255,255,255,0.8)';
	this._canvasSize = {width: 256, height: 256};
	this._image = new Image();
	var $root = this._$rootDiv = $('<div style="width:256px"></div>');//background-color: rgba(91,133,181,0.5)
	this._$table = $('<table width="256" style="font-size:40px;border-radius: 10px;background-color: #EFF0DC;box-shadow: inset 0 1px 1px rgba(0,0,0,.05);"></table>')
		.appendTo($root)
		.css({
			paddingTop: '10px',
			paddingBottom: '10px',
...
mono.extend(AVBillboardViewTemplate,Object,{
	setId: function(id){
		if(!id) return;
	...
	updateTable: function(data, id){
		...
		$table.empty();
		var i = 0, color, val, node;
		for(var p in data){
			if(i++>=4)break;
			color = '#00B38C';
			val = parseFloat(data[p]);
			if(val >= 13){
				color = '#D32F2F';
			}
			if(p === '电流'){
				node = main.sceneManager.getNodeByDataOrId(id);
				var alarmManager = main.sceneManager.getAlarmManager();
				if(val >= 13){
					var alarm = new ITAlarm(id+'123',id,it.AlarmSeverity.getByName('critical'),'电流过高',new Date());
					alarm.alarmTypeId = '物理告警';
					if (!alarmManager.getAlarm(id+'123')) {
	                    alarmManager.add(alarm);
	                }
				} else {
					alarmManager.removeAlarmsByDataOrId(id);
				}
			}
...
it.viewTemplate.AVBillboardViewTemplate = AVBillboardViewTemplate;