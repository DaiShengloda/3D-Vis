 ...
 */
'use strict';
var DEFAULT_URL = '';//./pdfdemo/test11.pdf';
  ...
  (function () {
    ...
    (function (root, factory) {
      ...
      function scrollIntoView(element, spot, skipOverflowHiddenElements) {
        ...
        if (!parent) {
          // console.error('offsetParent is not set -- cannot scroll');//pdf View 从body中移除了或是隐藏了!
          return;
        }
    ...
    (function (root, factory) {
      ...
      var PDFViewerApplication = function(){
        ...
        this.pageRotation = 0;
        this.isInitialViewSet = false;
        this.animationStartedPromise = null;
        ...
        this.preferenceDisablePageLabels = false;
      ...
      PDFViewerApplication.prototype = {
        ...
        validateFileURL : function validateFileURL(file) {
            try {
              ...
              if (this.HOSTED_VIEWER_ORIGINS.indexOf(viewerOrigin) >= 0) {
        ...
        webViewerOpenFileViaURL : function webViewerOpenFileViaURL(file) {
            if (file && file.lastIndexOf('file:', 0) === 0) {
              ...
              self.setTitleUsingUrl(file);
              var xhr = new XMLHttpRequest();
              xhr.onload = function() {
                self.open(new Uint8Array(xhr.response));
              };
              try {
                xhr.open('GET', file);
                ...
                self.error(mozL10n.get('loading_error', null, 'An error occurred while loading the PDF.'), e);
              }
              return;
            }
            if (file) {
        ...
        webViewerInitialized : function (){
            var file;
            var self = this;
            var queryString = document.location.search.substring(1);
            var params = parseQueryString(queryString);
            file = 'file' in params ? params.file : DEFAULT_URL;
            this.validateFileURL(file);
            var waitForBeforeOpening = [];
            var appConfig = self.appConfig;
            ...
            var PDFJS = pdfjsLib.PDFJS;
            if (self.preferencePdfBugEnabled) {
            ...
            appConfig.toolbar.scaleSelect.addEventListener('change', function() {
              ...
              self.pdfViewer.currentScaleValue = this.value;
            });
            appConfig.toolbar.presentationModeButton.addEventListener('click', function(e) {
              self.eventBus.dispatch('presentationmode');
            });
            appConfig.toolbar.openFile.addEventListener('click', function(e) {
              self.eventBus.dispatch('openfile');
            });
            appConfig.toolbar.print.addEventListener('click', function(e) {
              // self.eventBus.dispatch('print');
               window.print(self);
            });
            appConfig.toolbar.download.addEventListener('click', function(e) {
              self.eventBus.dispatch('download');
            });
            Promise.all(waitForBeforeOpening).then(function() {
              self.webViewerOpenFileViaURL(file);
            }).catch(function(reason) {
              self.error(mozL10n.get('loading_error', null, 'An error occurred while opening.'), reason);
            });
          },
        ...
        setTitle: function pdfViewSetTitle(title) {
          ...
          // document.title = title; //不改变title 2017-02-22
        },
        /**
        ...
        beforePrint: function pdfViewSetupBeforePrint() {
          ...
          if (!this.pdfViewer.pageViewsReady) {
            ...
            window.alert(notReadyMessage); //2017-08-02
            return;
          }
        ...
        bindEvents: function pdfViewBindEvents() {
          ...
          function webViewerPageRendered(e) {
            ...
            if (pageNumber === self.page) {
              var pageNumberInput = self.appConfig.toolbar.pageNumber;
            ...
            if (self.pdfSidebar.isThumbnailViewVisible) {
              var thumbnailView = self.pdfThumbnailViewer.getThumbnail(pageIndex);
              thumbnailView.setImage(pageView);
            }
            ...
            if (pageView.error) {
              self.error(mozL10n.get('rendering_error', null, 'An error occurred while rendering the page.'), pageView.error);
            }
          }
          ...
          function webViewerPageMode(e) {
            ...
            self.pdfSidebar.switchView(view, /* forceOpen = */
              true);
          }
          ...
          function webViewerNamedAction(e) {
            ...
            switch (action) {
              case 'GoToPage':
                self.appConfig.toolbar.pageNumber.select();
                break;
              case 'Find':
                if (!self.supportsIntegratedFind) {
                  self.findBar.toggle();
          ...
          function webViewerPresentationModeChanged(e) {
            ...
            self.pdfViewer.presentationModeState = switchInProgress ? PresentationModeState.CHANGING : active ? PresentationModeState.FULLSCREEN : PresentationModeState.NORMAL;
          }
          ...
          function webViewerSidebarViewChanged(e) {
            ...
            self.pdfRenderingQueue.isThumbnailViewEnabled = self.pdfSidebar.isThumbnailViewVisible;
            var store = self.store;
            if (!store || !self.isInitialViewSet) {
          ...
          function webViewerUpdateViewarea(e) {
            ...
            var href = self.pdfLinkService.getAnchorUrl(location.pdfOpenParams);
            self.appConfig.toolbar.viewBookmark.href = href;
            self.appConfig.secondaryToolbar.viewBookmarkButton.href = href;
            // Update the current bookmark in the browsing history.
            self.pdfHistory.updateCurrentBookmark(location.pdfOpenParams, location.pageNumber);
            // Show/hide the loading indicator in the page number input element.
            var pageNumberInput = self.appConfig.toolbar.pageNumber;
            var currentPage = self.pdfViewer.getPageView(self.page - 1);
          ...
          function webViewerResize() {
            if (self.initialized) {
              var currentScaleValue = self.pdfViewer.currentScaleValue;
          ...
          function webViewerHashchange(e) {
            if (self.pdfHistory.isHashChangeUnlocked) {
              var hash = e.hash;
              if (!hash) {
                return;
              }
              if (!self.isInitialViewSet) {
                ...
                self.pdfLinkService.setHash(hash);
              }
            }
          }
          ...
          webViewerFileInputChange = function webViewerFileInputChange(e) {
            ...
            if (!pdfjsLib.PDFJS.disableCreateObjectURL && typeof URL !== 'undefined' && URL.createObjectURL) {
              self.open(URL.createObjectURL(file));
            } else {
              ...
              fileReader.onload = function webViewerChangeFileReaderOnload(evt) {
                ...
                self.open(uint8Array);
              };
              fileReader.readAsArrayBuffer(file);
            }
            self.setTitleUsingUrl(file.name);
          ...
          function webViewerLocalized() {
            ...
            self.animationStartedPromise.then(function() {
              // Adjust the width of the zoom box to fit the content.
              // Note: If the window is narrow enough that the zoom box is not visible,
              //       we temporarily show it to be able to adjust its width.
              var container = self.appConfig.toolbar.scaleSelectContainer;
              if (container.clientWidth === 0) {
                container.setAttribute('style', 'display: inherit;');
              }
              if (container.clientWidth > 0) {
                var select = self.appConfig.toolbar.scaleSelect;
          ...
          function webViewerOpenFile() {
            var openFileInputName = self.appConfig.openFileInputName;
          ...
          function webViewerDocumentProperties() {
            self.pdfDocumentProperties.open();
          }

          function webViewerFind(e) {
            self.findController.executeCommand('find' + e.type, {
              query: e.query,
              phraseSearch: e.phraseSearch,
              caseSensitive: e.caseSensitive,
              highlightAll: e.highlightAll,
          ...
          function webViewerFindFromUrlHash(e) {
            self.findController.executeCommand('find', {
              query: e.query,
              phraseSearch: e.phraseSearch,
              caseSensitive: false,
              highlightAll: true,
          ...
          function webViewerPageChanging(e) {
            ...
            if (self.pdfSidebar.isThumbnailViewVisible) {
              self.pdfThumbnailViewer.scrollThumbnailIntoView(page);
            }
            // we need to update stats
            if (pdfjsLib.PDFJS.pdfBug && Stats.enabled) {
              var pageView = self.pdfViewer.getPageView(page - 1);
              if (pageView.stats) {
          ...
          function handleMouseWheel(evt) {
            ...
            if (evt.ctrlKey || evt.metaKey) {
              var support = self.supportedMouseWheelZoomModifierKeys;
        ...
        installDefaultEvent: function() {
          ...
          window.addEventListener('change', function webViewerChange(evt) {
            ...
            if (!files || files.length === 0 || !self.eventBus) {
              return;
            }
            self.eventBus.dispatch('fileinputchange', {
          ...
          window.addEventListener('click', function click(evt) {
            if (!self.secondaryToolbar || !self.secondaryToolbar.isOpen) {
              return;
            }
            var appConfig = self.appConfig;
            if (self.pdfViewer.containsElement(evt.target) || appConfig.toolbar.container.contains(evt.target) && evt.target !== appConfig.secondaryToolbar.toggleButton) {
              self.secondaryToolbar.close();
            }
          }, true);
            ...
            if (cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
              ...
              switch (evt.keyCode) {
                ...
                case 71:
                  // g
                  if (!self.supportsIntegratedFind) {
                    var findState = self.findController.state;
                    if (findState) {
                      self.findController.executeCommand('findagain', {
            ...
            if (cmd === 0) {
              ...
              switch (evt.keyCode) {
                ...
                case 27:
                  ...
                  if (!self.supportsIntegratedFind && self.findBar.opened) {
                    self.findBar.close();
                    handled = true;
                  }
                  break;
                ...
                case 36:
                  // home
                  if (isViewerInPresentationMode || self.page > 1) {
                    self.page = 1;
                    handled = true;
                ...
                case 35:
                  // end
                  if (isViewerInPresentationMode || self.page < self.pagesCount) {
                    self.page = self.pagesCount;
                    handled = true;
                    ensureViewerFocused = true;
                  }
          ...
          window.addEventListener('beforeprint', function beforePrint(evt) {
            // self.eventBus.dispatch('beforeprint');
            self.beforePrint(); //eventBus是全局的
          });
          window.addEventListener('afterprint', function afterPrint(evt) {
            // self.eventBus.dispatch('afterprint');
            self.afterPrint(); //eventBus是全局的
          });

          (function animationStartedClosure() {
            // The offsetParent is not set until the pdf.js iframe or object is visible.
            // Waiting for first animation.
            self.animationStartedPromise = new Promise(function(resolve) {
    ...
    (function (root, factory) {
      ...
      var print = window.print; //这里定义了一个全局的，
      window.print = function print(scope) {
        if (activeService) {
...
/**
 * 这样的话，只能整一个pdfViwe了，不仅仅是只能同时显示一个。两个交叉显示都不行，除非每次都重新初始化(没试，可能可以)
 */
function getViewerConfiguration(id) {
  var id = id || '';
  return {
    eventBus : new pdfjsWebLibs.pdfjsWebUIUtils.EventBus(),
    appContainer: document.body,
    mainContainer: document.getElementById('viewerContainer'+id),
    viewerContainer: document.getElementById('viewer'+id),
    // eventBus: null,
    // using global event bus with DOM events
    toolbar: {
      container: document.getElementById('toolbarViewer'+id),
      numPages: document.getElementById('numPages'+id),
      pageNumber: document.getElementById('pageNumber'+id),
      scaleSelectContainer: document.getElementById('scaleSelectContainer'+id),
      scaleSelect: document.getElementById('scaleSelect'+id),
      customScaleOption: document.getElementById('customScaleOption'+id),
      previous: document.getElementById('previous'+id),
      next: document.getElementById('next'+id),
      firstPage: document.getElementById('firstPage'+id),
      lastPage: document.getElementById('lastPage'+id),
      zoomIn: document.getElementById('zoomIn'+id),
      zoomOut: document.getElementById('zoomOut'+id),
      viewFind: document.getElementById('viewFind'+id),
      openFile: document.getElementById('openFile'+id),
      print: document.getElementById('print'+id),
      presentationModeButton: document.getElementById('presentationMode'+id),
      download: document.getElementById('download'+id),
      viewBookmark: document.getElementById('viewBookmark'+id)
    },
    secondaryToolbar: {
      toolbar: document.getElementById('secondaryToolbar'+id),
      toggleButton: document.getElementById('secondaryToolbarToggle'+id),
      toolbarButtonContainer: document.getElementById('secondaryToolbarButtonContainer'+id),
      presentationModeButton: document.getElementById('secondaryPresentationMode'+id),
      openFileButton: document.getElementById('secondaryOpenFile'+id),
      printButton: document.getElementById('secondaryPrint'+id),
      downloadButton: document.getElementById('secondaryDownload'+id),
      viewBookmarkButton: document.getElementById('secondaryViewBookmark'+id),
      firstPageButton: document.getElementById('firstPage'+id),
      lastPageButton: document.getElementById('lastPage'+id),
      pageRotateCwButton: document.getElementById('pageRotateCw'+id),
      pageRotateCcwButton: document.getElementById('pageRotateCcw'+id),
      toggleHandToolButton: document.getElementById('toggleHandTool'+id),
      documentPropertiesButton: document.getElementById('documentProperties'+id)
    },
    fullscreen: {
      contextFirstPage: document.getElementById('contextFirstPage'+id),
      contextLastPage: document.getElementById('contextLastPage'+id),
      contextPageRotateCw: document.getElementById('contextPageRotateCw'+id),
      contextPageRotateCcw: document.getElementById('contextPageRotateCcw'+id)
    },
    sidebar: {
      // Divs (and sidebar button)
      mainContainer: document.getElementById('mainContainer'+id),
      outerContainer: document.getElementById('outerContainer'+id),
      toggleButton: document.getElementById('sidebarToggle'+id),
      // Buttons
      thumbnailButton: document.getElementById('viewThumbnail'+id),
      outlineButton: document.getElementById('viewOutline'+id),
      attachmentsButton: document.getElementById('viewAttachments'+id),
      // Views
      thumbnailView: document.getElementById('thumbnailView'+id),
      outlineView: document.getElementById('outlineView'+id),
      attachmentsView: document.getElementById('attachmentsView'+id)
    },
    findBar: {
      bar: document.getElementById('findbar'+id),
      toggleButton: document.getElementById('viewFind'+id),
      findField: document.getElementById('findInput'+id),
    ...
    passwordOverlay: {
      overlayName: 'passwordOverlay'+id,
      container: document.getElementById('passwordOverlay'+id),
      label: document.getElementById('passwordText'+id),
      input: document.getElementById('password'+id),
      submitButton: document.getElementById('passwordSubmit'+id),
      cancelButton: document.getElementById('passwordCancel'+id)
    },
    documentProperties: {
      overlayName: 'documentPropertiesOverlay'+id,
      container: document.getElementById('documentPropertiesOverlay'+id),
...
function webViewerLoad(pdfViewerApplication,id) {
  var config = getViewerConfiguration(id);
  // window.PDFViewerApplication = pdfjsWebLibs.pdfjsWebApp.PDFViewerApplication;
  // pdfjsWebLibs.pdfjsWebApp.PDFViewerApplication.run(config);
  window.pdfViewerApplication = pdfViewerApplication;
  pdfViewerApplication.run(config);
  pdfViewerApplication.installDefaultEvent();
}
